    ...
    public class MazeRenderer : Component
    {
        ...
        public void Draw3D()
        {
            ...
            for (int x = min.X; x < max.X; x++)
            {
                ...
                {
                    if (!_mazeCells[x, y].hasStairsDown)
                        DrawFloor(new Vector3(x + 0.5f,y + 0.5f,0.01f) + transform.Position3D, new Vector3(0,0,0));
                    if (!Globals.DebugModeToggled && !_mazeCells[x, y].hasStairsUp)
